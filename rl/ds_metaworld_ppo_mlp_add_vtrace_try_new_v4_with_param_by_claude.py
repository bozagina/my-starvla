...
# ===== Structured Credit Assignment (SC-PPO) 超参数 =====
SC_ENABLE = int(os.environ.get("SC_ENABLE", "0")) == 1  # 是否启用 SC-PPO
SC_OUTCOME_ENABLE = int(os.environ.get("SC_OUTCOME_ENABLE", "1")) == 1  # 是否启用 Outcome 模型
SC_OUTCOME_LR = float(os.environ.get("SC_OUTCOME_LR", "3e-4"))  # Outcome 模型学习率
SC_OUTCOME_WARMUP = int(os.environ.get("SC_OUTCOME_WARMUP", "500"))  # Outcome 模型预训练步数
SC_OUTCOME_UPDATE_FREQ = int(os.environ.get("SC_OUTCOME_UPDATE_FREQ", "5"))  # 每 N 个训练步更新一次
SC_OUTCOME_FOCAL_ALPHA = float(os.environ.get("SC_OUTCOME_FOCAL_ALPHA", "0.25"))  # focal loss alpha
SC_OUTCOME_FOCAL_GAMMA = float(os.environ.get("SC_OUTCOME_FOCAL_GAMMA", "2.0"))  # focal loss gamma

SC_ADV_LR = float(os.environ.get("SC_ADV_LR", "3e-4"))  # StructuredAdv 模型学习率
SC_ADV_UPDATE_FREQ = int(os.environ.get("SC_ADV_UPDATE_FREQ", "5"))  # 每 N 个训练步更新一次
SC_LAM_PAIR = float(os.environ.get("SC_LAM_PAIR", "1e-3"))  # pair 项 L2 正则系数
SC_LAM_GAUGE = float(os.environ.get("SC_LAM_GAUGE", "1e-3"))  # gauge fixing 正则系数
SC_USE_GAUGE_LOSS = int(os.environ.get("SC_USE_GAUGE_LOSS", "1")) == 1  # 是否使用 gauge 约束

SC_TOPK_K = int(os.environ.get("SC_TOPK_K", "8"))  # Top-K counterfactual baseline 的 K
SC_CREDIT_UPDATE = int(os.environ.get("SC_CREDIT_UPDATE", "0")) == 1  # 是否使用分通道 credit 更新
SC_CREDIT_START_STEP = int(os.environ.get("SC_CREDIT_START_STEP", "1000"))  # 从第几步开始 credit 更新

# Structured Credit 网络架构超参
SC_STATE_HIDDEN = int(os.environ.get("SC_STATE_HIDDEN", "256"))  # state encoder hidden size
SC_ACTION_EMB_DIM = int(os.environ.get("SC_ACTION_EMB_DIM", "64"))  # action embedding dim
SC_UNARY_HIDDEN = int(os.environ.get("SC_UNARY_HIDDEN", "128"))  # unary head hidden size
SC_PAIR_HIDDEN = int(os.environ.get("SC_PAIR_HIDDEN", "128"))  # pair head hidden size

# ================================================================
# Structured Credit: helper modules
# ================================================================
PAIRS_4D = [(0, 1), (0, 2), (0, 3), (1, 2), (1, 3), (2, 3)]


class StructuredAdvantageNet(nn.Module):
    """
    Unary + pairwise advantage decomposition for MultiDiscrete actions.
    Outputs:
        u: [B, D]
        p: [B, n_pairs]
        A: [B] scalar total advantage
    """
    ...
    def __init__(
        ...
        self.pairs = PAIRS_4D[: (action_dim * (action_dim - 1)) // 2]
        ...
        self.embeddings = nn.ModuleList(
            [nn.Embedding(n_action_bins, action_emb_dim) for _ in range(action_dim)]
        )
        self.unary_heads = nn.ModuleList(
            [
                nn.Sequential(
                    nn.Linear(state_hidden + action_emb_dim, unary_hidden),
                    nn.ReLU(),
                    nn.Linear(unary_hidden, 1),
                )
        ...
        self.pair_heads = nn.ModuleList(
            [
                nn.Sequential(
                    nn.Linear(state_hidden + 2 * action_emb_dim, pair_hidden),
                    nn.ReLU(),
    ...
    def forward_components(self, obs: torch.Tensor, actions: torch.Tensor):
        """
        obs: [B, state_dim]
        actions: [B, D] int64
        """
        h = self.state_encoder(obs)  # [B, Hs]
        embs = [emb(actions[:, i]) for i, emb in enumerate(self.embeddings)]  # list of [B, E]

        unary_out = []
        for i in range(self.action_dim):
            inp = torch.cat([h, embs[i]], dim=-1)
            unary_out.append(self.unary_heads[i](inp).squeeze(-1))
        u = torch.stack(unary_out, dim=-1)  # [B, D]

        pair_out = []
        for head, (i, j) in zip(self.pair_heads, self.pairs):
            inp = torch.cat([h, embs[i], embs[j]], dim=-1)
            pair_out.append(head(inp).squeeze(-1))
        p = torch.stack(pair_out, dim=-1) if len(pair_out) > 0 else torch.zeros(
            (obs.shape[0], 0), device=obs.device, dtype=obs.dtype
        )

        A = u.sum(dim=-1) + (p.sum(dim=-1) if p.numel() > 0 else 0.0)
        return u, p, A

    def compute_C_from_components(self, u: torch.Tensor, p: torch.Tensor) -> torch.Tensor:
        """
        Build per-dimension contribution C_i from unary and pair terms.
        """
        B, D = u.shape
...
class TrainerActor(TrainerActorCom):
    def __init__(self, rank, world_size, replay_buffer, seed=SEED):
        ...
        self.ema_state = None
        # Structured credit components (initialized after model is created)
        self.sc_adv_net: Optional[StructuredAdvantageNet] = None
        self.sc_adv_opt: Optional[torch.optim.Optimizer] = None
        self.sc_topk_k = SC_TOPK_K
        self.sc_last_metrics: Dict[str, float] = {}
    ...
    def setup_deepspeed_group(self, master_addr, master_port):
        ...
        # Structured credit networks (standalone optimizer, not in DeepSpeed engine)
        if SC_ENABLE:
            device_sc = next(self.model.parameters()).device
            self.sc_adv_net = StructuredAdvantageNet(
                state_dim=STATE_DIM,
                action_dim=ACTION_DIM,
                n_action_bins=N_ACTION_BINS,
                state_hidden=SC_STATE_HIDDEN,
                action_emb_dim=SC_ACTION_EMB_DIM,
                unary_hidden=SC_UNARY_HIDDEN,
                pair_hidden=SC_PAIR_HIDDEN,
            ).to(device_sc)
            self.sc_adv_opt = torch.optim.Adam(self.sc_adv_net.parameters(), lr=SC_ADV_LR)
            print(f"Trainer {self.rank}: StructuredAdvantageNet 初始化完成（SC_ENABLE=True）。")
...
# Bonus Time (Loss A) helpers
    ...
    # ===========================
    # Structured Credit (SC-PPO)
    # ===========================
    def _sc_compute_credit(
        self,
        obs: torch.Tensor,
        ...
        """
        Compute structured advantage, optional credit-based policy loss, and train the advantage net.
        """
        if (self.sc_adv_net is None) or (self.sc_adv_opt is None):
            return {}

        B = obs.shape[0]
        device = obs.device
        topk_k = int(self.sc_topk_k)

        # --- Advantage network forward on sampled actions ---
        u, p, A_pred = self.sc_adv_net.forward_components(obs, act_token)
        C_actual = self.sc_adv_net.compute_C_from_components(u, p)  # [B, D]

        # --- Top-K counterfactual baseline ---
        with torch.no_grad():
            prob_old = torch.softmax(logits_old, dim=-1)  # [B, D, K]
            topk_prob, topk_idx = prob_old.topk(topk_k, dim=-1)  # [B, D, K]
            topk_prob = topk_prob / (topk_prob.sum(dim=-1, keepdim=True) + 1e-8)

        b_list = []
        # For each dimension, replace that dim with top-k tokens and evaluate C_i
        for dim in range(ACTION_DIM):
            actions_i = act_token.unsqueeze(1).expand(-1, topk_k, -1).reshape(B * topk_k, ACTION_DIM)
            actions_i[:, dim] = topk_idx[:, dim, :].reshape(-1)
            obs_i = obs.unsqueeze(1).expand(-1, topk_k, -1).reshape(B * topk_k, obs.shape[-1])
            with torch.no_grad():
                u_k, p_k, _ = self.sc_adv_net.forward_components(obs_i, actions_i)
                C_k = self.sc_adv_net.compute_C_from_components(u_k, p_k)  # [B*K, D]
                C_i_k = C_k[:, dim].reshape(B, topk_k)
            b_i = (topk_prob[:, dim, :] * C_i_k).sum(dim=-1)  # [B]
            b_list.append(b_i)
        b = torch.stack(b_list, dim=-1)  # [B, D]
        A_dim = C_actual - b  # [B, D]

        # --- Advantage network loss (targets from normalized_adv) ---
        target_adv = normalized_adv.detach()
        adv_mse = F.mse_loss(A_pred, target_adv)
        pair_reg = p.pow(2).mean()
        gauge_loss = torch.tensor(0.0, device=device)
        if SC_USE_GAUGE_LOSS:
            gauge_loss = (u.mean(dim=0).pow(2).sum())
            if p.numel() > 0:
                gauge_loss = gauge_loss + p.mean(dim=0).pow(2).sum()
        adv_loss = adv_mse + SC_LAM_PAIR * pair_reg + SC_LAM_GAUGE * gauge_loss

        # Train adv net at configured frequency
        adv_updated = 0
        if (current_step % max(1, SC_ADV_UPDATE_FREQ)) == 0:
            self.sc_adv_opt.zero_grad()
            adv_loss.backward()
            self.sc_adv_opt.step()
            adv_updated = 1

        # --- Credit-based policy loss (Variant B) ---
        # Joint ratio on sampled actions
        logp_joint_new = logp_new.sum(dim=-1)
        logp_joint_old = logp_old.sum(dim=-1)
        ratio_joint = torch.exp(logp_joint_new - logp_joint_old)  # [B]
        ratio_clip = ratio_joint.clamp(1.0 - CLIP_EPS, 1.0 + CLIP_EPS)
        A_gate = A_pred.detach()
        w = torch.where(
            A_gate >= 0,
            torch.min(ratio_joint, ratio_clip),
            torch.max(ratio_joint, ratio_clip),
        )
        policy_credit_loss = -(w.detach() * (A_dim.detach() * logp_new).sum(dim=-1)).mean()
        clip_ratio = ((ratio_joint < (1.0 - CLIP_EPS)) | (ratio_joint > (1.0 + CLIP_EPS))).float().mean().item()

        # Diagnostics
        unary_energy = u.abs().mean().item()
        pair_energy = p.abs().mean().item() if p.numel() > 0 else 0.0
        pair_ratio = pair_energy / (unary_energy + pair_energy + 1e-8)

        metrics = {
            "sc_adv_loss": float(adv_loss.item()),
            "sc_adv_mse": float(adv_mse.item()),
            "sc_adv_pair_reg": float(pair_reg.item()),
            "sc_adv_gauge_loss": float(gauge_loss.item()),
            "sc_adv_updated": float(adv_updated),
            "sc_unary_energy": unary_energy,
            "sc_pair_energy": pair_energy,
            "sc_pair_ratio": pair_ratio,
            "sc_clip_ratio": clip_ratio,
        }

        # Per-dimension credit stats
        for d in range(min(ACTION_DIM, 4)):
            metrics[f"sc_A_dim_mean_d{d}"] = float(A_dim[:, d].mean().item())
            metrics[f"sc_A_dim_std_d{d}"] = float(A_dim[:, d].std().item())
            metrics[f"sc_C_mean_d{d}"] = float(C_actual[:, d].mean().item())
            metrics[f"sc_u_mean_d{d}"] = float(u[:, d].mean().item())
        for idx, (i, j) in enumerate(PAIRS_4D[: p.shape[-1]]):
            metrics[f"sc_p_mean_p{idx}"] = float(p[:, idx].mean().item())

        metrics["sc_credit_active"] = float(1.0 if (SC_CREDIT_UPDATE and current_step >= SC_CREDIT_START_STEP) else 0.0)
        metrics["sc_topk_k"] = float(topk_k)

        return {
            "policy_credit_loss": policy_credit_loss,
    ...
    async def run_training_epoch(self) -> Tuple[float, float, float, float, float, Dict[str, float], int, float, float, float, Dict[str, float]]:
        ...
        # reset SC metrics container
        if not SC_ENABLE:
            self.sc_last_metrics = {}
        ...
        for i in range(num_updates_in_epoch):
            ...
            if self.global_step < POLICY_TRAIN_START_STEP:
                ...
                # Structured Credit: compute credit-based loss & adv training
                sc_out = None
                sc_metrics_local: Dict[str, float] = {}
                if SC_ENABLE:
                    sc_out = self._sc_compute_credit(
                        mini_obs,
                        mini_act_token,
                        mini_logits_old,
                        normalized_adv,
                    ...
                    if sc_out and isinstance(sc_out, dict):
                        sc_metrics_local = sc_out.get("metrics", {})
                    self.sc_last_metrics = sc_metrics_local
                ...
                credit_active = SC_ENABLE and SC_CREDIT_UPDATE and (self.global_step >= SC_CREDIT_START_STEP) and sc_out
                if credit_active:
                    policy_loss = sc_out["policy_credit_loss"]
                    clip_ratio = sc_out["clip_ratio"]
                else:
        ...
        # Append latest structured-credit diagnostics (if any)
        if SC_ENABLE and self.sc_last_metrics:
            perf_metrics.update(self.sc_last_metrics)
...
def main():
    ...
    while global_step < TRAIN_ITERS:
        ...
        if current_time - last_log_time > LOG_INTERVAL_SECONDS:
            ...
            pm0 = perf_metrics_list[0] if len(perf_metrics_list) > 0 else {}
            ...
            # SC-PPO 状态简报（如果启用）
            if SC_ENABLE and 'outcome_acc' in pm0:
                outcome_status = f"OutcomeAcc:{pm0['outcome_acc']:.3f}"
                if 'sc_pair_ratio' in pm0:
                    pair_ratio_status = f"PairRatio:{pm0['sc_pair_ratio']:.3f}"
                else:
                    pair_ratio_status = "PairRatio:N/A"
                
                credit_active = "✓" if (SC_CREDIT_UPDATE and global_step >= SC_CREDIT_START_STEP) else "✗"
                
                print(f"  [SC-PPO] {outcome_status} | {pair_ratio_status} | Credit:{credit_active} | "
                      f"SuccRate:{rollout_success_rate:.3f}")
            ...
            # ===== SC-PPO 诊断指标（新增）=====
            if SC_ENABLE:
                # === 1. Outcome Model 质量指标 ===
                if 'outcome_loss' in pm0:
                    log_metrics['SC/Outcome_Loss'] = pm0['outcome_loss']
                    log_metrics['SC/Outcome_Acc'] = pm0['outcome_acc']
                    log_metrics['SC/Outcome_Pos_Rate'] = pm0['outcome_pos_rate']          # 真实成功率
                    log_metrics['SC/Outcome_Pred_Pos_Rate'] = pm0['outcome_pred_pos_rate']  # 预测成功率
                    
                    # 关键诊断：预测偏差（应该接近 0）
                    pred_bias = pm0['outcome_pred_pos_rate'] - pm0['outcome_pos_rate']
                    log_metrics['SC/Outcome_Prediction_Bias'] = pred_bias
                    log_metrics['SC/Outcome_Prediction_Bias_Abs'] = abs(pred_bias)
                    
                    # 是否健康（Acc > 0.7 且偏差 < 0.1）
                    is_healthy = (pm0['outcome_acc'] > 0.7) and (abs(pred_bias) < 0.1)
                    log_metrics['SC/Outcome_Is_Healthy'] = 1.0 if is_healthy else 0.0
                
                # === 2. Structured Advantage 能量分布 ===
                if 'sc_adv_loss' in pm0:
                    log_metrics['SC/Adv_Loss_Total'] = pm0['sc_adv_loss']
                    log_metrics['SC/Adv_Loss_MSE'] = pm0['sc_adv_mse']
                    log_metrics['SC/Adv_Loss_Pair_Reg'] = pm0['sc_adv_pair_reg']
                    log_metrics['SC/Adv_Loss_Gauge'] = pm0['sc_adv_gauge_loss']
                    
                    log_metrics['SC/Energy_Unary'] = pm0['sc_unary_energy']
                    log_metrics['SC/Energy_Pair'] = pm0['sc_pair_energy']
                    log_metrics['SC/Energy_Pair_Ratio'] = pm0['sc_pair_ratio']  # 关键：pair/(unary+pair)
                    
                    # 能量比例健康度（0.2-0.5 为理想范围）
                    pair_ratio = pm0['sc_pair_ratio']
                    pair_ratio_healthy = 1.0 if (0.2 <= pair_ratio <= 0.5) else 0.0
                    log_metrics['SC/Energy_Pair_Ratio_Healthy'] = pair_ratio_healthy
                
                # === 3. Per-Dimension Credit Assignment ===
                # 每个维度的 credit 均值（应该有明显差异，说明分配合理）
                a_dim_means = []
                for d in range(4):
                    key_mean = f'sc_A_dim_mean_d{d}'
                    key_std = f'sc_A_dim_std_d{d}'
                    if key_mean in pm0:
                        log_metrics[f'SC/Credit_Mean_D{d}'] = pm0[key_mean]
                        log_metrics[f'SC/Credit_Std_D{d}'] = pm0[key_std]
                        a_dim_means.append(pm0[key_mean])
                
                # Credit 分化度（标准差，越大说明维度区分越明显）
                if len(a_dim_means) == 4:
                    credit_std = float(np.std(a_dim_means))
                    log_metrics['SC/Credit_Differentiation'] = credit_std  # 关键指标
                    
                    # 是否有效区分（std > 0.1 说明维度 credit 有实质差异）
                    log_metrics['SC/Credit_Has_Differentiation'] = 1.0 if credit_std > 0.1 else 0.0
                
                # === 4. Per-Dimension Contribution (C_i) ===
                for d in range(4):
                    key_c = f'sc_C_mean_d{d}'
                    key_u = f'sc_u_mean_d{d}'
                    if key_c in pm0:
                        log_metrics[f'SC/Contribution_C_D{d}'] = pm0[key_c]
                        log_metrics[f'SC/Unary_U_D{d}'] = pm0[key_u]
                
                # === 5. Pairwise Interaction Terms ===
                # Pairs order: [(0,1),(0,2),(0,3),(1,2),(1,3),(2,3)]
                pair_names = ['01', '02', '03', '12', '13', '23']
                pair_means = []
                for p_idx, pair_name in enumerate(pair_names):
                    key_p = f'sc_p_mean_p{p_idx}'
                    if key_p in pm0:
                        log_metrics[f'SC/Pair_P{pair_name}'] = pm0[key_p]
                        pair_means.append(abs(pm0[key_p]))
                
                # Pair 能量分布均匀度（方差，用于判断是否某些 pair 占主导）
                if len(pair_means) == 6:
                    pair_energy_std = float(np.std(pair_means))
                    pair_energy_mean = float(np.mean(pair_means))
                    log_metrics['SC/Pair_Energy_Std'] = pair_energy_std
                    log_metrics['SC/Pair_Energy_Mean'] = pair_energy_mean
                    
                    # 归一化变异系数（CV = std/mean）
                    if pair_energy_mean > 1e-6:
                        cv = pair_energy_std / pair_energy_mean
                        log_metrics['SC/Pair_Energy_CV'] = cv
                
                # === 6. Training Status Indicators ===
                # 用于判断各阶段是否按预期工作
                log_metrics['SC/Is_Policy_Training'] = 1.0 if global_step >= POLICY_TRAIN_START_STEP else 0.0
                log_metrics['SC/Is_Credit_Active'] = 1.0 if (SC_CREDIT_UPDATE and global_step >= SC_CREDIT_START_STEP) else 0.0
                
                # Outcome 预训练完成度（0-1）
                if SC_OUTCOME_WARMUP > 0:
                    outcome_warmup_progress = min(1.0, global_step / float(SC_OUTCOME_WARMUP))
                    log_metrics['SC/Outcome_Warmup_Progress'] = outcome_warmup_progress
                else:
                    log_metrics['SC/Outcome_Warmup_Progress'] = 1.0
                
                # === 7. 数据质量指标（基于成功率推断）===
                # 当前成功率（用于判断 Outcome Model 是否有足够的正样本）
                current_success_rate = rollout_success_rate
                log_metrics['SC/Data_Success_Rate'] = current_success_rate
                log_metrics['SC/Data_Has_Sufficient_Positives'] = 1.0 if current_success_rate > 0.05 else 0.0
                
                # === 8. Composite Health Score ===
                # 综合健康度评分（0-1，越接近 1 越健康）
                health_score = 0.0
                health_count = 0
                
                # Outcome 健康（权重 0.3）
                if 'outcome_acc' in pm0:
                    outcome_ok = pm0['outcome_acc'] > 0.7
                    pred_bias_ok = abs(pm0['outcome_pred_pos_rate'] - pm0['outcome_pos_rate']) < 0.15
                    if outcome_ok and pred_bias_ok:
                        health_score += 0.3
                    health_count += 1
                ...
                # 数据质量（权重 0.2）
                if current_success_rate > 0.03:
                    health_score += 0.2
                    health_count += 1
                
                log_metrics['SC/Health_Score'] = health_score if health_count > 0 else 0.0
                ...
                if 'outcome_acc' in pm0:
                    if pm0['outcome_acc'] < 0.6:
                        debug_flags['outcome_low_acc'] = 1.0
                    if abs(pm0['outcome_pred_pos_rate'] - pm0['outcome_pos_rate']) > 0.2:
                        debug_flags['outcome_large_bias'] = 1.0
                    if pm0['outcome_pred_pos_rate'] < 0.01 or pm0['outcome_pred_pos_rate'] > 0.99:
                        debug_flags['outcome_collapsed'] = 1.0
                
                if 'sc_pair_ratio' in pm0:
                    if pm0['sc_pair_ratio'] > 0.7:
                        debug_flags['pair_dominates'] = 1.0
                    if pm0['sc_pair_ratio'] < 0.05:
                        debug_flags['pair_too_weak'] = 1.0
                
                if len(a_dim_means) == 4 and float(np.std(a_dim_means)) < 0.02:
                    debug_flags['credit_no_diff'] = 1.0
                
                for flag_name, flag_val in debug_flags.items():
                    log_metrics[f'SC/Debug_{flag_name}'] = flag_val
