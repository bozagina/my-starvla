...
PAIR_MIN_SUCCESS_POOL = 32         # success 池太小则不挖掘 pair
...
# ===== Bonus Time (Loss A) 超参数 =====
BONUS_TRIGGER_MIN_CLUSTERS = 2      # 触发 bonus 时，Paired-Queue 中 success cluster 覆盖至少多少种（v1 防 mode collapse）

BONUS_TRIGGER_MIN_PAIRS = 256      # paired_queue 至少积累到多少（每个 rank 本地）才允许触发
BONUS_BATCH_PACKS = 32             # 每次 bonus 取多少个 PairPack（每个 pack 含 window steps）
BONUS_MAX_STEPS = 4096             # bonus batch 最大总步数（展开后会截断/随机子采样）
...
# Bonus Time (Loss A) helpers
    ...
    async def _should_run_bonus_globally(self) -> bool:
        """避免多卡不同步：只有当所有 rank 都满足触发条件时才跑 bonus。
        v1：额外要求 Paired-Queue 中 success cluster 覆盖数 >= BONUS_TRIGGER_MIN_CLUSTERS（防 mode collapse）。
        
        注意：TrainerActor 使用 async loop，必须使用 await 方式获取远程结果，避免阻塞事件循环。
        """
        ...
        local_ok = 1 if (local_pairs >= int(BONUS_TRIGGER_MIN_PAIRS) and local_cov >= int(BONUS_TRIGGER_MIN_CLUSTERS)) else 0
        if (not distributed.is_available()) or (not distributed.is_initialized()):
            return bool(local_ok)
        t = torch.tensor([local_ok], device=next(self.model.parameters()).device, dtype=torch.int32)
        # 用 MIN：只要有一张卡不满足，就全部不跑
    ...
    async def _run_bonus_time_if_ready(self) -> Dict[str, float]:  # (Part3) Bonus Time：从 Paired-Queue 取 pack 做 Loss A 更新
        ...
        if not await self._should_run_bonus_globally():
            metrics['bonus/ran'] = 0.0
            return metrics
    
        # 取 paired packs（consume=True：用掉以免反复过拟合同一批）
        packs = await self.replay_buffer.sample_paired_packs.remote(BONUS_BATCH_PACKS, True, True)
    ...
    async def run_training_epoch(self) -> Tuple[float, float, float, float, float, Dict[str, float], int, float, float, float, Dict[str, float]]:
        ...
        try:
            ...
            # bonus 不允许影响主训练稳定性
            bonus_metrics = {'bonus/ran': 0.0, 'bonus/error': 1.0}
            if self.rank == 0:
    ...
    while global_step < TRAIN_ITERS:
        ...
        if current_time - last_log_time > LOG_INTERVAL_SECONDS:
            ...
            for k, v in pm0.items():
                if isinstance(v, (int, float)) and (k.startswith("bonus/") or k.startswith("vtrace/")):
                    log_metrics[k] = v
            # 友好别名
