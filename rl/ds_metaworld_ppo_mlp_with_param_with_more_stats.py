        ...
        while True:
            ...
            try:
                ...
                with torch.inference_mode():
                    ...
                    # 计算行为策略 log-prob
                    # ✅ 关键：保持 logp_mu 为 [B, ACTION_DIM]，不做 sum
                    # 原因：每个动作维度独立采样，保留维度信息可以：
                    #   1) 计算更精确的 ratio（逐维度）
                    #   2) 用于后续的诊断分析（per-dimension statistics）
                    #   3) 实验表明 sum 后效果会变差
                    logp_mu = dist.log_prob(action_tokens)  # [B, ACTION_DIM]
                    # if logp_mu.dim() > 1:
                    #     logp_mu = logp_mu.sum(dim=-1)      # (B,) - 不使用 sum！
    ...
    async def run_training_epoch(self) -> Tuple[float, float, float, float, float, Dict[str, float], int, float, float, float, Dict[str, float]]:
        ...
        for i in range(num_updates_in_epoch):
            ...
            if self.global_step < POLICY_TRAIN_START_STEP:
                ...
                ent = torch.mean(dist.entropy())  # [B, ACTION_DIM] -> scalar
...
def main():
    ...
    log_dir = f"runs/MetaWorld/{BENCHMARK}/MLP_DS_PPO_{clip_suffix}_{int(time.time())}_seed{SEED}"
        ...
        if swanlab is None:
            ...
            try:
                swanlab.init(
                    ...
                    experiment_name=f"{BENCHMARK}_MLP_DS_PPO_rollout_{NUM_ROLLOUT_WORKERS}_without_sum_in_action_dim_{CLIP_MODE}_{int(time.time())}_seed{SEED}",
