...
ENABLE_OUTCOME_MODEL = int(os.environ.get("ENABLE_OUTCOME_MODEL", "0")) == 1
ENABLE_STRUCTURED_ADV = int(os.environ.get("ENABLE_STRUCTURED_ADV", "0")) == 1
ENABLE_CREDIT_UPDATE = int(os.environ.get("ENABLE_CREDIT_UPDATE", "0")) == 1
STRUCT_TOPK_K = int(os.environ.get("STRUCT_TOPK_K", "8"))
STRUCT_LAM_PAIR = float(os.environ.get("STRUCT_LAM_PAIR", "1e-3"))
STRUCT_USE_GAUGE_LOSS = int(os.environ.get("STRUCT_USE_GAUGE_LOSS", "0")) == 1
STRUCT_LAM_GAUGE = float(os.environ.get("STRUCT_LAM_GAUGE", "1e-3"))
OUTCOME_LR = float(os.environ.get("OUTCOME_LR", "3e-5"))
ADV_LR = float(os.environ.get("ADV_LR", "3e-5"))
OUTCOME_BATCH_SIZE = int(os.environ.get("OUTCOME_BATCH_SIZE", "1024"))
STRUCT_MIN_SUCCESS_EP = int(os.environ.get("STRUCT_MIN_SUCCESS_EP", "8"))
STRUCT_MIN_FAILURE_EP = int(os.environ.get("STRUCT_MIN_FAILURE_EP", "8"))
STRUCT_ENABLE_GAUGE_UNARY_ONLY = True
...
class OutcomeModel(nn.Module):
    def __init__(self, state_dim: int, action_dim: int, n_action_bins: int, hidden_dim: int = 256, emb_dim: int = 64):
        super().__init__()
        ...
        self.action_embeddings = nn.ModuleList(
            [nn.Embedding(n_action_bins, emb_dim) for _ in range(action_dim)]
        )
        final_in = hidden_dim + action_dim * emb_dim
    ...
    def forward(self, obs: torch.Tensor, actions: torch.Tensor) -> torch.Tensor:
        h = self.state_mlp(obs)
        embeds = []
        for i in range(actions.shape[1]):
            emb_i = self.action_embeddings[i](actions[:, i])
            embeds.append(emb_i)
        a_emb = torch.cat(embeds, dim=-1)
        x = torch.cat([h, a_emb], dim=-1)
        logit = self.head(x).squeeze(-1)
        return logit


class StructuredAdvNet(nn.Module):
    def __init__(self, state_dim: int, action_dim: int, n_action_bins: int, hidden_dim: int = 256, emb_dim: int = 64):
        super().__init__()
        self.state_mlp = nn.Sequential(
            nn.Linear(state_dim, hidden_dim),
    ...
    def _encode(self, obs: torch.Tensor, actions: torch.Tensor) -> Tuple[torch.Tensor, List[torch.Tensor]]:
        h = self.state_mlp(obs)
        embeds = []
        for i in range(actions.shape[1]):
            emb_i = self.action_embeddings[i](actions[:, i])
    ...
    def forward_components(self, obs: torch.Tensor, actions: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        h, embeds = self._encode(obs, actions)
        unary_list = []
        for i in range(self.action_dim):
            x_i = torch.cat([h, embeds[i]], dim=-1)
            u_i = self.unary_heads[i](x_i).squeeze(-1)
            unary_list.append(u_i)
        u = torch.stack(unary_list, dim=-1)
        pair_list = []
        for k, (i, j) in enumerate(self.pairs):
            x_ij = torch.cat([h, embeds[i], embeds[j]], dim=-1)
            p_ij = self.pair_heads[k](x_ij).squeeze(-1)
            pair_list.append(p_ij)
        p = torch.stack(pair_list, dim=-1) if pair_list else torch.zeros(
            obs.shape[0], 0, device=obs.device, dtype=h.dtype
        )
        a_total = u.sum(dim=-1) + p.sum(dim=-1)
        return u, p, a_total
    ...
    def compute_C(self, obs: torch.Tensor, actions: torch.Tensor) -> torch.Tensor:
        u, p, _ = self.forward_components(obs, actions)
        return self.compute_C_from_components(u, p)


def compute_structured_adv_and_credit(
    adv_net: StructuredAdvNet,
    obs: torch.Tensor,
    actions: torch.Tensor,
    logits_old: torch.Tensor,
    topk_k: int,
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    u, p, a_total = adv_net.forward_components(obs, actions)
    C_actual = adv_net.compute_C_from_components(u, p)
    prob_old = F.softmax(logits_old, dim=-1)
    topk_prob, topk_idx = prob_old.topk(topk_k, dim=-1)
    topk_prob = topk_prob / (topk_prob.sum(dim=-1, keepdim=True) + 1e-8)
    B, D, K = topk_idx.shape
    baselines = []
    for i in range(D):
        actions_i_topk = actions.unsqueeze(1).expand(B, K, D).clone()
        actions_i_topk[:, :, i] = topk_idx[:, i, :]
        obs_rep = obs.unsqueeze(1).expand(B, K, obs.shape[-1]).reshape(B * K, obs.shape[-1])
        act_flat = actions_i_topk.reshape(B * K, D)
        u_k, p_k, _ = adv_net.forward_components(obs_rep, act_flat)
        C_k = adv_net.compute_C_from_components(u_k, p_k)
        C_i_k = C_k[:, i].reshape(B, K)
        b_i = (topk_prob[:, i, :] * C_i_k).sum(dim=-1)
        baselines.append(b_i)
    b = torch.stack(baselines, dim=-1)
    A_dim = C_actual - b
    return u, p, a_total, C_actual, b, A_dim
...
# ===========================
    ...
    def sample_outcome_batch(self, max_steps: int):
        success_ids = [int(eid) for eid in self._success_eps if int(eid) in self._episode_traj]
        failure_ids = [int(eid) for eid in self._failure_eps if int(eid) in self._episode_traj]
        if len(success_ids) < STRUCT_MIN_SUCCESS_EP or len(failure_ids) < STRUCT_MIN_FAILURE_EP:
            return None
        max_steps = int(max_steps)
        if max_steps <= 0:
            return None
        target_pos = max_steps // 2
        target_neg = max_steps - target_pos
        obs_list = []
        act_list = []
        label_list = []
        pos_steps = 0
        neg_steps = 0
        while (pos_steps < target_pos or neg_steps < target_neg) and (success_ids or failure_ids):
            if pos_steps < target_pos and success_ids:
                eid = random.choice(success_ids)
                traj = self._episode_traj.get(int(eid))
                if not traj:
                    try:
                        success_ids.remove(eid)
                    except ValueError:
                        pass
                else:
                    states = [e.obs for e in traj]
                    actions = [e.action_token for e in traj]
                    remain = target_pos - pos_steps
                    if len(states) > remain:
                        idx = np.random.choice(len(states), size=remain, replace=False)
                        states = [states[i] for i in idx]
                        actions = [actions[i] for i in idx]
                    obs_list.append(np.stack(states, axis=0).astype(np.float32))
                    act_list.append(np.stack(actions, axis=0).astype(np.int64))
                    label_list.append(np.ones(len(states), dtype=np.float32))
                    pos_steps += len(states)
            if neg_steps < target_neg and failure_ids:
                eid = random.choice(failure_ids)
                traj = self._episode_traj.get(int(eid))
                if not traj:
                    try:
                        failure_ids.remove(eid)
                    except ValueError:
                        pass
                else:
                    states = [e.obs for e in traj]
                    actions = [e.action_token for e in traj]
                    remain = target_neg - neg_steps
                    if len(states) > remain:
                        idx = np.random.choice(len(states), size=remain, replace=False)
                        states = [states[i] for i in idx]
                        actions = [actions[i] for i in idx]
                    obs_list.append(np.stack(states, axis=0).astype(np.float32))
                    act_list.append(np.stack(actions, axis=0).astype(np.int64))
                    label_list.append(np.zeros(len(states), dtype=np.float32))
                    neg_steps += len(states)
            if (pos_steps >= target_pos or not success_ids) and (neg_steps >= target_neg or not failure_ids):
                break
        if not obs_list:
            return None
...
class TrainerActor(TrainerActorCom):
    def __init__(self, rank, world_size, replay_buffer, seed=SEED):
        ...
        self.ema_state = None
        self.outcome_model: Optional[OutcomeModel] = None
        self.outcome_optimizer: Optional[torch.optim.Optimizer] = None
        self.adv_net: Optional[StructuredAdvNet] = None
        self.adv_optimizer: Optional[torch.optim.Optimizer] = None
    ...
    def setup_deepspeed_group(self, master_addr, master_port):
        ...
        if ENABLE_OUTCOME_MODEL:
            self.outcome_model = OutcomeModel(
                state_dim=STATE_DIM,
                action_dim=ACTION_DIM,
                n_action_bins=N_ACTION_BINS,
            ).to(device)
            self.outcome_optimizer = torch.optim.Adam(self.outcome_model.parameters(), lr=OUTCOME_LR)
        if ENABLE_STRUCTURED_ADV:
            self.adv_net = StructuredAdvNet(
                state_dim=STATE_DIM,
                action_dim=ACTION_DIM,
                n_action_bins=N_ACTION_BINS,
            ).to(device)
            self.adv_optimizer = torch.optim.Adam(self.adv_net.parameters(), lr=ADV_LR)
    ...
    async def _train_outcome_and_adv(self) -> Dict[str, float]:
        metrics: Dict[str, float] = {}
        if not ENABLE_OUTCOME_MODEL or self.outcome_model is None or self.outcome_optimizer is None:
            return metrics
        try:
            sampled = await self.replay_buffer.sample_outcome_batch.remote(OUTCOME_BATCH_SIZE)
        except Exception:
            return metrics
        if sampled is None:
            return metrics
        obs_np, act_np, labels_np = sampled
        device = next(self.model.parameters()).device
        obs = torch.from_numpy(obs_np).to(device=device, dtype=TORCH_DTYPE)
        actions = torch.from_numpy(act_np).to(device=device, dtype=torch.long)
        labels = torch.from_numpy(labels_np).to(device=device, dtype=torch.float32)
        self.outcome_model.train()
        logits = self.outcome_model(obs, actions)
        pos_frac = float(labels.mean().item())
        if pos_frac > 0.0 and pos_frac < 1.0:
            pos_weight_value = (1.0 - pos_frac) / (pos_frac + 1e-8)
        else:
            pos_weight_value = 1.0
        pos_weight = torch.tensor(pos_weight_value, device=device, dtype=logits.dtype)
        loss_outcome = F.binary_cross_entropy_with_logits(logits, labels, pos_weight=pos_weight)
        self.outcome_optimizer.zero_grad()
        loss_outcome.backward()
        self.outcome_optimizer.step()
        metrics["adv/outcome_loss"] = float(loss_outcome.item())
        metrics["adv/outcome_pos_frac"] = float(pos_frac)
        if ENABLE_STRUCTURED_ADV and self.adv_net is not None and self.adv_optimizer is not None:
            self.adv_net.train()
            with torch.no_grad():
                q_hat = logits
                q_hat_centered = q_hat - q_hat.mean()
            u, p, a_total = self.adv_net.forward_components(obs, actions)
            target = q_hat_centered.detach()
            loss_mse = F.mse_loss(a_total, target)
            loss_pair = STRUCT_LAM_PAIR * p.pow(2).mean()
            loss_gauge = torch.tensor(0.0, device=device, dtype=a_total.dtype)
            if STRUCT_USE_GAUGE_LOSS and STRUCT_ENABLE_GAUGE_UNARY_ONLY:
                mean_unary = u.mean(dim=0)
                loss_gauge = STRUCT_LAM_GAUGE * mean_unary.pow(2).mean()
            loss_adv_total = loss_mse + loss_pair + loss_gauge
            self.adv_optimizer.zero_grad()
            loss_adv_total.backward()
            self.adv_optimizer.step()
            metrics["adv/adv_loss_total"] = float(loss_adv_total.item())
            metrics["adv/adv_loss_mse"] = float(loss_mse.item())
            metrics["adv/adv_loss_pair_l2"] = float(loss_pair.item())
            metrics["adv/adv_loss_gauge"] = float(loss_gauge.item())
        return metrics
...
# Bonus Time (Loss A) helpers
    ...
    async def run_training_epoch(self) -> Tuple[float, float, float, float, float, Dict[str, float], int, float, float, float, Dict[str, float]]:
        ...
        do_diag = False
        adv_aux_metrics: Dict[str, float] = {}
        if ENABLE_OUTCOME_MODEL:
            try:
                adv_aux_metrics = await self._train_outcome_and_adv()
            except Exception:
                adv_aux_metrics = {}
        ...
        epoch_unary_energy: List[float] = []
        epoch_pair_energy: List[float] = []
        epoch_pair_ratio: List[float] = []
        epoch_A_dim_means: List[List[float]] = [[] for _ in range(ACTION_DIM)]
        epoch_A_dim_vars: List[List[float]] = [[] for _ in range(ACTION_DIM)]
        num_pairs_struct = ACTION_DIM * (ACTION_DIM - 1) // 2
        epoch_pair_means: List[List[float]] = [[] for _ in range(num_pairs_struct)]
        ...
        for i in range(num_updates_in_epoch):
            ...
            if self.global_step < POLICY_TRAIN_START_STEP:
                ...
                b_s = None
                A_dim_s = None
                if ENABLE_STRUCTURED_ADV and self.adv_net is not None:
                    with torch.no_grad():
                        u_s, p_s, a_total_s, C_actual_s, b_s, A_dim_s = compute_structured_adv_and_credit(
                            self.adv_net,
                            mini_obs,
                            mini_act_token,
                            mini_logits_old,
                    ...
                    unary_energy = u_s.abs().mean()
                    pair_energy = p_s.abs().mean()
                    pair_ratio = pair_energy / (unary_energy + pair_energy + 1e-8)
                    epoch_unary_energy.append(float(unary_energy.item()))
                    epoch_pair_energy.append(float(pair_energy.item()))
                    epoch_pair_ratio.append(float(pair_ratio.item()))
                    for d in range(ACTION_DIM):
                        a_d = A_dim_s[:, d]
                        epoch_A_dim_means[d].append(float(a_d.mean().item()))
                        epoch_A_dim_vars[d].append(float(a_d.var(unbiased=False).item()))
                    num_pairs_local = p_s.shape[1]
                    for k in range(num_pairs_local):
                        epoch_pair_means[k].append(float(p_s[:, k].mean().item()))
                if ENABLE_CREDIT_UPDATE and ENABLE_STRUCTURED_ADV and self.adv_net is not None and CLIP_MODE == "clip" and A_dim_s is not None and a_total_s is not None:
                    logp_joint = logp.sum(dim=-1)
                    logp_old_joint = logp_old.sum(dim=-1)
                    ratio_joint = torch.exp(logp_joint - logp_old_joint)
                    ratio_clip = ratio_joint.clamp(1.0 - CLIP_EPS, 1.0 + CLIP_EPS)
                    A_gate = a_total_s.detach()
                    w_joint = torch.where(
                        A_gate >= 0.0,
                        torch.minimum(ratio_joint, ratio_clip),
                        torch.maximum(ratio_joint, ratio_clip),
                    )
                    loss_pi_credit = -(w_joint.detach() * (A_dim_s.detach() * logp).sum(dim=-1)).mean()
                    policy_loss = loss_pi_credit
                    pos_mask = A_gate >= 0.0
                    neg_mask = ~pos_mask
                    clipped_high = (ratio_joint > ratio_clip) & pos_mask
                    clipped_low = (ratio_joint < ratio_clip) & neg_mask
                    is_clipped_joint = clipped_high | clipped_low
                    clip_ratio = float(is_clipped_joint.float().mean().item())
                else:
        ...
        if isinstance(adv_aux_metrics, dict) and len(adv_aux_metrics) > 0:
            for k, v in adv_aux_metrics.items():
                try:
        ...
        if epoch_unary_energy:
            perf_metrics["adv/unary_energy"] = float(np.mean(epoch_unary_energy))
            perf_metrics["adv/pair_energy"] = float(np.mean(epoch_pair_energy)) if epoch_pair_energy else 0.0
            denom = (perf_metrics["adv/unary_energy"] + perf_metrics["adv/pair_energy"])
            perf_metrics["adv/pair_ratio"] = float(perf_metrics["adv/pair_energy"] / (denom + 1e-8))
        for d in range(ACTION_DIM):
            if epoch_A_dim_means[d]:
                perf_metrics[f"adv/A_dim_mean_{d}"] = float(np.mean(epoch_A_dim_means[d]))
            if epoch_A_dim_vars[d]:
                perf_metrics[f"adv/A_dim_var_{d}"] = float(np.mean(epoch_A_dim_vars[d]))
        for k in range(num_pairs_struct):
            if epoch_pair_means[k]:
                perf_metrics[f"adv/pair_mean_{k}"] = float(np.mean(epoch_pair_means[k]))
    ...
    while global_step < TRAIN_ITERS:
        ...
        if current_time - last_log_time > LOG_INTERVAL_SECONDS:
            ...
            for k, v in pm0.items():
                if isinstance(v, (int, float)) and (k.startswith("bonus/") or k.startswith("vtrace/") or k.startswith("pair/") or k.startswith("adv/")):
                    log_metrics[k] = v
            # 友好别名
