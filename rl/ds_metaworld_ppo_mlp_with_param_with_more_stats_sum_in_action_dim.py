...
# 为了让 Ray 能看到所有可用的 GPU，我们在脚本开头设置（可被命令行参数覆盖）。
...
# 画图工具
from rl.plot_utils import make_policy_prob_diag_image, make_old_new_prob_scatter_image

# ================================================================
# 0. 超参数与配置 (已修改为单任务学习，并对齐 ds_meta 关键超参便于对比)
# ================================================================
# # MetaWorld 单任务设置
# METAWORLD_TASKS = ["sweep-into-v3"]  # 单任务学习：只使用 reach-v3
# BENCHMARK = "MetaWorld_sweep_into_v3"
...
def compute_policy_surrogate(
    clip_mode: str,
    ratio: torch.Tensor,
    adv_unsqueezed: torch.Tensor,  # ✅ 名称保持兼容，但现在是 [B] 不需要 unsqueeze
    ...
    ✅ 修正：ratio 和 adv_unsqueezed 现在都是 [B] (joint)，不再是 [B, D]
    """
    # ✅ 运行时检查
    assert ratio.ndim == 1, f"Ratio must be 1D (joint), got {ratio.shape}"
    assert adv_unsqueezed.ndim == 1, f"Advantage must be 1D, got {adv_unsqueezed.shape}"
    
    surr1 = ratio * adv_unsqueezed  # [B] * [B] = [B]
    ...
    if clip_mode == "soft_clip" or clip_mode == "soft_clip_alpha-0" or clip_mode == "soft_clip_alpha-0-5" or clip_mode == "soft_clip_alpha-1" or clip_mode == "soft_clip_alpha-2":
        ...
        # 动态 alpha：alpha * (1 + sigmoid(-A))，负优势时更强抑制
    ...
    if clip_mode in ("sapo_soft_clip", "sapo", "sapo_gate"):
        ...
        # ✅ 修正：adv_unsqueezed 现在是 [B]，不需要 expand
        tau = torch.where(adv_unsqueezed > 0, 
                         torch.full_like(adv_unsqueezed, tau_pos),
                         torch.full_like(adv_unsqueezed, tau_neg))
        ...
        surr_sapo = gate * adv_unsqueezed  # [B] * [B] = [B]
...
class Experience:
    ...
    behaviour_logp: float = 0.0   # ✅ 修正：Joint log μ(a_t | s_t), scalar (sum over action dims)
...
class ReplayBufferActor:
    ...
    def sample_sequences(
        ...
        # 检查过滤后是否还有足够数据
        filtered_size = sum(len(traj) for traj, _, _ in buffer_list)
        ...
        for idx in indices:
            ...
            total_steps_collected += len(traj)
        ...
        for traj, done, last_obs in traj_batch:
            ...
            # ✅ 修正：behaviour_logp 现在是 scalar (joint)
            batch_logp_list.append(np.array([e.behaviour_logp for e in traj], dtype=np.float32))  # [T]
        ...
        for i, (s, a_token, r_val, logits, _, logp_mu, step_discount, policy_ver) in enumerate(traj_segment):
            traj.append(
                Experience(
                    ...
                    behaviour_logp=float(logp_mu),  # ✅ 修正：Joint log-prob (scalar)
...
class EvaluationWorkerActor(BaseWorkerActor):
    ...
    def run(self):
        try:
            # 使用确定性种子
            debug_log(f"EvaluationWorker {self.original_wid} run() 启动，准备 reset 环境")
            obs, info = self._reset_and_select_env()
            while True:
        ...
        while True:
            ...
            try:
                ...
                with torch.inference_mode():
                    ...
                    # 计算行为策略 log-prob
                    # ✅ 修正：MultiDiscrete 动作空间下，需要计算 joint log-prob
                    # 因为各维度独立采样：log p(a|s) = Σ_d log p(a_d|s)
                    logp_mu_per_dim = dist.log_prob(action_tokens)  # [B, ACTION_DIM]
                    logp_mu = logp_mu_per_dim.sum(dim=-1)  # [B] - Joint log-prob
                    
                    # ✅ 运行时检查：确保 shape 正确
                    assert logp_mu_per_dim.ndim == 2 and logp_mu_per_dim.shape[1] == 4, \
                        f"Per-dim logp shape error: {logp_mu_per_dim.shape}, expected [B, 4]"
                    assert logp_mu.ndim == 1, \
                        f"Joint logp must be 1D, got {logp_mu.shape}"
...
class TrainerActor(TrainerActorCom):
    ...
    async def dump_diagnostic_data(
        ...
        with torch.no_grad():
            ...
            # ✅ 修正：计算 joint log-prob
            dist_old = torch.distributions.Categorical(logits=logits_old_sample)
            logp_old_per_dim = dist_old.log_prob(act_token_sample)  # [N, action_dim]
            logp_old = logp_old_per_dim.sum(dim=-1)  # [N] - Joint
            
            dist_new = torch.distributions.Categorical(logits=action_logits)
            logp_new_per_dim = dist_new.log_prob(act_token_sample)  # [N, action_dim]
            logp_new = logp_new_per_dim.sum(dim=-1)  # [N] - Joint
            
            # 计算 joint ratio
            ratio = torch.exp(logp_new - logp_old)  # [N] - Joint
            logrho = logp_new - logp_old  # [N] - Joint
    ...
    def _compute_diagnostic_metrics(
        ...
        with torch.no_grad():
            ...
            # ==================== 预处理 ====================
            # ✅ 修正：ratio 现在已经是 [B] (joint)，不需要 reshape
            assert ratio.ndim == 1, f"Input ratio must be 1D (joint), got {ratio.shape}"
            assert advantage.ndim == 1, f"Input advantage must be 1D, got {advantage.shape}"
            assert ratio.shape == advantage.shape, \
                f"Ratio {ratio.shape} and advantage {advantage.shape} shape mismatch"
            
            ratio_flat = ratio  # [B]
            adv_expanded = advantage  # [B]
            ...
            new_mask = staleness_ver <= NEW_THRESHOLD  # [B]
            old_mask = staleness_ver >= OLD_THRESHOLD  # [B]
            
            # ==================== A. 分桶组成（Bucket Composition）====================
            # A1) 绝对分桶占比
            metrics['staleness_old_frac_abs'] = old_mask.float().mean().item()
            metrics['staleness_new_frac_abs'] = new_mask.float().mean().item()
            
            # A2) 旧桶内的陈旧度分布
            if old_mask.any():
                old_gaps = staleness_ver[old_mask]
            ...
            new_mask_ratio = staleness_ratio <= NEW_RATIO_THRESHOLD
            old_mask_ratio = staleness_ratio >= OLD_RATIO_THRESHOLD
            
            metrics['staleness_old_frac_ratio'] = old_mask_ratio.float().mean().item()
            metrics['staleness_new_frac_ratio'] = new_mask_ratio.float().mean().item()
            
            # ✅ ratio 现在是 [B]，mask 不需要扩展
            ...
            if clip_mode == "clip":  # Hard clip
                ...
                if clip_mode in ("soft_clip", "soft_clip_alpha-1", "soft_clip_alpha-2"):
                    # w = (1/max(ρ, 1/ρ))^alpha ，引入动态 alpha: alpha*(1+sigmoid(-A))
            ...
            u_grad_proxy = torch.abs(u * adv_expanded)  # [B] - |u * A|
    ...
    async def _data_fetching_loop(self):
        ...
        while True:
            try:
                ...
                # time.sleep(0.4)  # 模拟处理v-trace的耗时
                t_sample_start = time.time()
                # 精确采样模式：告诉 Buffer 我们至少需要 super_batch_size 这么多步数
                ...
                # 诊断字段
                policy_ver_flat = np.concatenate(policy_ver_list, axis=0)
                insert_step_flat = np.concatenate(insert_step_list, axis=0)
                ...
                # 将诊断字段转为 tensor
                policy_ver_t = torch.tensor(policy_ver_flat, dtype=torch.int32, device=device)
                insert_step_t = torch.tensor(insert_step_flat, dtype=torch.int32, device=device)
                ...
                self.next_ready_batch = {
                    ...
                    # ========== 新增：诊断字段 ==========
                    'policy_version': policy_ver_t,  # 行为策略版本
                    'insert_step': insert_step_t,    # 插入步数（用于计算 age）
                    
                    # 暂存原始列表供未来扩展
                    'obs_seq_list': obs_seq_list,
                    'act_seq_list': act_seq_list,
                    'rew_seq_list': rew_seq_list,
                    # ... 其他字段暂时不需要，等做 V-trace 时再加
    ...
    async def run_training_epoch(self) -> Tuple[float, float, float, float, float, Dict[str, float], int, float, float, float, Dict[str, float]]:
        ...
        # ========== 新增：诊断字段 ==========
        policy_version_t = current_batch['policy_version']
        insert_step_t = current_batch['insert_step']

        # =================================================================
        # 关键修正: 对 Flatten 后的序列数据进行 Shuffle
        # 现在的 obs_t 等数据是按轨迹顺序排列的 (Traj1_Step1, Traj1_Step2...)
        # 直接切片做 Mini-batch 会导致严重的数据相关性，破坏 SGD 的 IID 假设。
        # =================================================================
        ...
        adv_t = adv_t[indices]
        logits_old_t = logits_old_t[indices]
        v_targ_t = v_targ_t[indices]
        # 诊断字段也需要 shuffle
        ...
        # 我们在采样时已经多采了数据，所以这里 current_batch_size 应该总是 > super_batch_size
        num_updates_in_epoch = ACCUMULATION_STEPS
        
        if current_batch_size < self.super_batch_size:
            print(f"[Warning] Rank {self.rank}: 数据不足! {current_batch_size} < {self.super_batch_size}。这将导致死锁风险。")
        ...
        for i in range(num_updates_in_epoch):
            ...
            # ✅ 运行时检查：验证 logits shape [B, ACTION_DIM, N_ACTION_BINS]
            assert action_logits.shape == (mini_obs.shape[0], ACTION_DIM, N_ACTION_BINS), \
                f"action_logits shape error: expected {(mini_obs.shape[0], ACTION_DIM, N_ACTION_BINS)}, got {action_logits.shape}"
            # ✅ 验证 action_token shape [B, ACTION_DIM]
            assert mini_act_token.shape == (mini_obs.shape[0], ACTION_DIM), \
                f"mini_act_token shape error: expected {(mini_obs.shape[0], ACTION_DIM)}, got {mini_act_token.shape}"
            ...
            if self.global_step < POLICY_TRAIN_START_STEP:
                ...
                logp_per_dim = dist.log_prob(mini_act_token)  # [B, ACTION_DIM]
                logp = logp_per_dim.sum(dim=-1)  # [B] - Joint log-prob
                
                # ✅ 运行时检查：验证 log-prob shape
                assert logp_per_dim.shape == (mini_obs.shape[0], ACTION_DIM), \
                    f"logp_per_dim shape error: expected {(mini_obs.shape[0], ACTION_DIM)}, got {logp_per_dim.shape}"
                assert logp.shape == (mini_obs.shape[0],), \
                    f"logp (joint) shape error: expected {(mini_obs.shape[0],)}, got {logp.shape}"

                with torch.no_grad():
                    dist_old = torch.distributions.Categorical(logits=mini_logits_old)
                    logp_old_per_dim = dist_old.log_prob(mini_act_token)  # [B, ACTION_DIM]
                    logp_old = logp_old_per_dim.sum(dim=-1)  # [B] - Joint log-prob
                    
                    # ✅ 运行时检查：验证 old log-prob shape
                    assert logp_old_per_dim.shape == logp_per_dim.shape, \
                        f"logp_old_per_dim shape mismatch: {logp_old_per_dim.shape} vs {logp_per_dim.shape}"
                    assert logp_old.shape == logp.shape, \
                        f"logp_old (joint) shape mismatch: {logp_old.shape} vs {logp.shape}"

                # ✅ 修正：KL 和 Entropy 也需要 sum over action dims
                kl_div_per_dim = kl.kl_divergence(dist_old, dist)  # [B, ACTION_DIM]
                kl_div_joint = kl_div_per_dim.sum(dim=-1)  # [B] - Joint KL
                kl_div = torch.mean(kl_div_joint).item()
                kl_loss = KL_COEF * torch.mean(kl_div_joint)

                # ✅ 修正：ratio 现在是 [B] (joint)，不需要 unsqueeze advantage
                ratio = torch.exp(logp - logp_old)  # [B] - Joint ratio
                
                # ✅ 运行时检查：shape 和数值稳定性
                assert ratio.shape == normalized_adv.shape, \
                    f"Ratio {ratio.shape} and advantage {normalized_adv.shape} shape mismatch"
                assert torch.isfinite(ratio).all(), \
                    f"Non-finite ratio detected! logp range: [{logp.min():.2f}, {logp.max():.2f}], logp_old range: [{logp_old.min():.2f}, {logp_old.max():.2f}]"
                assert torch.isfinite(logp).all() and torch.isfinite(logp_old).all(), \
                    f"Non-finite log-prob detected! logp: {logp[~torch.isfinite(logp)]}, logp_old: {logp_old[~torch.isfinite(logp_old)]}"
                
                # ✅ 收集 log-prob 统计（用于诊断）
                if i == 0:  # 只在第一个 mini-batch 收集
                    logp_per_dim_stats.append({
                        'mean': logp_per_dim.mean().item(),
                        'std': logp_per_dim.std().item(),
                    })
                    logp_joint_stats.append({
                        'mean': logp.mean().item(),
                        'std': logp.std().item(),
                    })
                ...
                if i == 0 and self.global_step >= POLICY_TRAIN_START_STEP:
                    # ✅ Debug 日志：记录 log-prob 统计（每 100 步打印一次）
                    if self.rank == 0 and self.global_step % 100 == 0:
                        print(f"[DEBUG] Step {self.global_step} | "
                              f"logp_per_dim stats: mean={logp_per_dim.mean().item():.3f}, std={logp_per_dim.std().item():.3f}, "
                              f"logp (joint) stats: mean={logp.mean().item():.3f}, std={logp.std().item():.3f}, "
                              f"ratio stats: mean={ratio.mean().item():.3f}, median={ratio.median().item():.3f}, "
                              f"max={ratio.max().item():.3f}")
                ...
                # ✅ 修复：使用 normalized_adv 替换未定义的 adv_unsqueezed
                if diag_outside_clip_ratio is None:
                    with torch.no_grad():
                        # ✅ ratio 和 normalized_adv 都是 [B]，无需 expand
                        outside = ((ratio < 1 - diag_eps) | (ratio > 1 + diag_eps)).float().mean()
                        dead_grad = (((ratio > 1 + diag_eps) & (normalized_adv > 0)) | ((ratio < 1 - diag_eps) & (normalized_adv < 0))).float().mean()
                ...
                policy_loss, clip_ratio = compute_policy_surrogate(
                    clip_mode=CLIP_MODE,
                    ratio=ratio,
                    adv_unsqueezed=normalized_adv,  # ✅ 修正：不需要 unsqueeze
                    clip_params=CLIP_PARAMS,
                )

                # ✅ 修正：Entropy 需要 sum over action dims
                ent_per_dim = dist.entropy()  # [B, ACTION_DIM]
                ent_joint = ent_per_dim.sum(dim=-1)  # [B] - Joint entropy
                ent = torch.mean(ent_joint)  # scalar
        ...
        # ✅ 新增：添加 log-prob 统计（用于诊断 sum(dim=-1) 的影响）
        if logp_per_dim_stats:
            perf_metrics['logp_per_dim_mean'] = logp_per_dim_stats[0]['mean']
            perf_metrics['logp_per_dim_std'] = logp_per_dim_stats[0]['std']
        if logp_joint_stats:
            perf_metrics['logp_joint_mean'] = logp_joint_stats[0]['mean']
            perf_metrics['logp_joint_std'] = logp_joint_stats[0]['std']
...
def main():
    ...
    global REPLAY_RECENT_FRAC, REPLAY_MAX_VERSION_GAP
    ...
    # 控制 Ray 对象存储占用，避免预占用过大
    object_store_size_gb = 32  # 分配的GB数，如需调整可修改此值
    ...
    swanlab.init(
        ...
        experiment_name=f"{BENCHMARK}_MLP_DS_PPO_sum_in_action_dim_{CLIP_MODE}_{int(time.time())}_seed{SEED}",
        description=f"MetaWorld MLP DeepSpeed PPO Training - {BENCHMARK} -{CLIP_MODE} (Seed: {SEED})",
    ...
    while global_step < TRAIN_ITERS:
        ...
        if current_time - last_log_time > LOG_INTERVAL_SECONDS:
            ...
            # ✅ 新增：记录 per-dim 和 joint logp 统计（用于诊断 sum(dim=-1) 的影响）
            # 这些指标在 debug 日志中已记录，这里将其持久化到 tensorboard
            if 'logp_per_dim_mean' in perf_metrics_list[0]:
                log_metrics['Debug/LogP_PerDim_Mean'] = perf_metrics_list[0]['logp_per_dim_mean']
                log_metrics['Debug/LogP_PerDim_Std'] = perf_metrics_list[0]['logp_per_dim_std']
                log_metrics['Debug/LogP_Joint_Mean'] = perf_metrics_list[0]['logp_joint_mean']
                log_metrics['Debug/LogP_Joint_Std'] = perf_metrics_list[0]['logp_joint_std']
